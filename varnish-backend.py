...
#        print "host_port_timeout=%s" % host_port_timeout
#        print "secret2=%s" % secret
    ...
    def fetch(self, command):
        ...
        logging.debug('RECV: %s: %d byte(s) %s' % (status,length,content[:30]))
        self.read_eager()
...
# Backends
    def backend_list(self):
        ...
        backend.list
            List backends
        """
        return self.fetch('backend.list')[1]

    def backend_set_health(self, backendname, backendstatus):
        """
        backend.set_health 
        """
        return self.fetch('backend.set_health %s %s' % (backendname, backendstatus))
